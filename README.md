...
- `Riff`: a DFS-generated sequence of fretboard positions with target-tone/motion/melodic-interval tags, inferred `Finger` suggestions for fretted notes, and a derived physical/anatomical movement cost.
- `physical_cost`, `infer_fingering`, `anatomical_cost`, `RiffScorer`, and `PhysicalDistanceScorer`: physical ranking primitives where adjacent fret movement stays cheap, large fret jumps are penalized quadratically, string crossing remains linear, and inferred Finger `1..=4` assignments add heavy penalties for impossible hand stretches.
